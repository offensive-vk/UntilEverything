...
async function createKeyValuePair<K, V>(
  key: K,
  value: V,
): Promise<KeyValuePair<K, V>> {
...
(async () => {
  ...
  const numberStringPair = await createKeyValuePair<number, string>(
    1,
    "TypeScript",
  );

  // Create a KeyValuePair with string key and boolean value
  const stringBooleanPair = await createKeyValuePair<string, boolean>(
    "isTrue",
    true,
  );
